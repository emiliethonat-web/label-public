  ...
  function attachListeners() {
    document.querySelectorAll('.check-item input[type=checkbox]').forEach(cb => {
      cb.addEventListener('change', () => {
        ...
        if (cb.checked) {
          const grp = cb.closest('.q-block');
          if (grp) {
            const isNone = ((cb.closest('.check-item').textContent || '').trim().toLowerCase().indexOf('aucun') === 0);
            grp.querySelectorAll('.check-item input[type=checkbox]').forEach(o => {
              if (o === cb) return;
              const oNone = ((o.closest('.check-item').textContent || '').trim().toLowerCase().indexOf('aucun') === 0);
              if ((isNone || oNone) && o.checked) {
                o.checked = false;
                o.closest('.check-item').classList.remove('selected');
              }
            });
            if (isNone) {
              grp.querySelectorAll('input[type=text], input[type=number]').forEach(inp => { inp.value = ''; });
            }
          }
        }
        refreshConditionals();
